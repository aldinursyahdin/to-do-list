import { useState } from "react"
import TodoTypes from "../Todo"
import TodoService from "../TodoService"

const ListToDo = () => {
  const [todos, setTodos] = useState<TodoTypes[]>(TodoService.getTodos())

  const handleDeleteTodo = (id: number) => {
    TodoService.deleteTodo(id)
    setTodos((prevTodo) => prevTodo.filter((todo) => todo.id !== id))
     window.location.reload()
    
  }

  const handleCompleted = (id: number, text: string, completed: boolean) => {
    const updateTodo = TodoService.updateTodo({
      id,
      text,
      completed: !completed,
    })
    setTodos((prevTodos) => prevTodos.map((todo) => (todo.id === id ? updateTodo : todo)))
     window.location.reload();
  }

  const truncateText = (text: string, maxChars: number) => {
    return text.length > maxChars ? `${text.slice(0, maxChars)}...` : text
  }

  return (
    <div>
      {todos?.length > 0 &&
        todos.map((todo) => (
          <div className="items" key={todo.id}>
            {todo.completed == false && (
              <div className="flex flex-row justify-center items-center p-2 gap-4 w-full h-[52px] bg-white rounded-lg shadow-custom-dark mb-[12px]">
                <svg onClick={() => handleCompleted(todo.id, todo.text, todo.completed)} className="cursor-pointer" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M2.25 12C2.25 6.615 6.615 2.25 12 2.25C17.385 2.25 21.75 6.615 21.75 12C21.75 17.385 17.385 21.75 12 21.75C6.615 21.75 2.25 17.385 2.25 12ZM15.61 10.186C15.67 10.1061 15.7134 10.0149 15.7377 9.91795C15.762 9.82098 15.7666 9.72014 15.7514 9.62135C15.7361 9.52257 15.7012 9.42782 15.6489 9.3427C15.5965 9.25757 15.5276 9.18378 15.4463 9.12565C15.3649 9.06753 15.2728 9.02624 15.1753 9.00423C15.0778 8.98221 14.9769 8.97991 14.8785 8.99746C14.7801 9.01501 14.6862 9.05205 14.6023 9.10641C14.5184 9.16077 14.4462 9.23135 14.39 9.314L11.154 13.844L9.53 12.22C9.38783 12.0875 9.19978 12.0154 9.00548 12.0188C8.81118 12.0223 8.62579 12.101 8.48838 12.2384C8.35097 12.3758 8.27225 12.5612 8.26882 12.7555C8.2654 12.9498 8.33752 13.1378 8.47 13.28L10.72 15.53C10.797 15.6069 10.8898 15.6662 10.992 15.7036C11.0942 15.7411 11.2033 15.7559 11.3118 15.7469C11.4202 15.738 11.5255 15.7055 11.6201 15.6519C11.7148 15.5982 11.7967 15.5245 11.86 15.436L15.61 10.186Z"
                    fill="#F5F5F4"
                  />
                </svg>

                <div className=" w-[688px]">
                  <p className="font-medium text-sm leading-5 text-left">{truncateText(todo.text, 85)}</p>
                </div>

                <svg onClick={() => handleDeleteTodo(todo.id)} className="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M13 10H7M19 10C19 11.1819 18.7672 12.3522 18.3149 13.4442C17.8626 14.5361 17.1997 15.5282 16.364 16.364C15.5282 17.1997 14.5361 17.8626 13.4442 18.3149C12.3522 18.7672 11.1819 19 10 19C8.8181 19 7.64778 18.7672 6.55585 18.3149C5.46392 17.8626 4.47177 17.1997 3.63604 16.364C2.80031 15.5282 2.13738 14.5361 1.68508 13.4442C1.23279 12.3522 1 11.1819 1 10C1 7.61305 1.94821 5.32387 3.63604 3.63604C5.32387 1.94821 7.61305 1 10 1C12.3869 1 14.6761 1.94821 16.364 3.63604C18.0518 5.32387 19 7.61305 19 10Z"
                    stroke="#E11D48"
                    stroke-width="1.5"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                </svg>
              </div>
            )}
          </div>
        ))}

      {todos?.length > 0 &&
        todos.map((todo) => (
          <div className="items" key={todo.id}>
            {todo.completed == true && (
              <div className="flex flex-row justify-center items-center p-2 gap-4 w-full h-[52px] bg-white rounded-lg shadow-custom-dark mb-[12px] opacity-50">
                <svg onClick={() => handleCompleted(todo.id, todo.text, todo.completed)} className="cursor-pointer" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M2.25 12C2.25 6.615 6.615 2.25 12 2.25C17.385 2.25 21.75 6.615 21.75 12C21.75 17.385 17.385 21.75 12 21.75C6.615 21.75 2.25 17.385 2.25 12ZM15.61 10.186C15.67 10.1061 15.7134 10.0149 15.7377 9.91795C15.762 9.82098 15.7666 9.72014 15.7514 9.62135C15.7361 9.52257 15.7012 9.42782 15.6489 9.3427C15.5965 9.25757 15.5276 9.18378 15.4463 9.12565C15.3649 9.06753 15.2728 9.02624 15.1753 9.00423C15.0778 8.98221 14.9769 8.97991 14.8785 8.99746C14.7801 9.01501 14.6862 9.05205 14.6023 9.10641C14.5184 9.16077 14.4462 9.23135 14.39 9.314L11.154 13.844L9.53 12.22C9.38783 12.0875 9.19978 12.0154 9.00548 12.0188C8.81118 12.0223 8.62579 12.101 8.48838 12.2384C8.35097 12.3758 8.27225 12.5612 8.26882 12.7555C8.2654 12.9498 8.33752 13.1378 8.47 13.28L10.72 15.53C10.797 15.6069 10.8898 15.6662 10.992 15.7036C11.0942 15.7411 11.2033 15.7559 11.3118 15.7469C11.4202 15.738 11.5255 15.7055 11.6201 15.6519C11.7148 15.5982 11.7967 15.5245 11.86 15.436L15.61 10.186Z"
                    fill="#059669"
                  />
                </svg>

                <div className=" w-[688px]">
                  <p className="font-medium text-sm leading-5 text-left">{truncateText(todo.text, 85)}</p>
                </div>

                <svg onClick={() => handleDeleteTodo(todo.id)} className="cursor-pointer" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M13 10H7M19 10C19 11.1819 18.7672 12.3522 18.3149 13.4442C17.8626 14.5361 17.1997 15.5282 16.364 16.364C15.5282 17.1997 14.5361 17.8626 13.4442 18.3149C12.3522 18.7672 11.1819 19 10 19C8.8181 19 7.64778 18.7672 6.55585 18.3149C5.46392 17.8626 4.47177 17.1997 3.63604 16.364C2.80031 15.5282 2.13738 14.5361 1.68508 13.4442C1.23279 12.3522 1 11.1819 1 10C1 7.61305 1.94821 5.32387 3.63604 3.63604C5.32387 1.94821 7.61305 1 10 1C12.3869 1 14.6761 1.94821 16.364 3.63604C18.0518 5.32387 19 7.61305 19 10Z"
                    stroke="#E11D48"
                    stroke-width="1.5"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                </svg>
              </div>
            )}
          </div>
        ))}

      {todos?.length == 0 && (
        <div className="flex flex-row justify-center items-start p-9  w-full h-[112px]   bg-white shadow-custom-dark rounded-lg mb-10">
          <p className="text-sm font-medium  flex items-center text-[#44403C]">Nothing to-do yet.</p>
        </div>
      )}
    </div>
  )
}

export default ListToDo

